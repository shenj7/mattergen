#!/usr/bin/env python3
# Bulk modulus oracle via QHA workflow.
# Usage: python calc_bulk_modulus.py your_structure.cif [optional_outdir]
...
def _to_atoms(obj) -> Atoms:
    """
    Accepts a CIF path, pymatgen Structure, or ASE Atoms and returns ASE Atoms.
    This lets us call the oracle directly from in-memory structures during guidance.
    """
    ...
    raise TypeError(f"Unsupported input type for bulk modulus calc: {type(obj)}")


def _fit_bulk_modulus(ev_rows: list[tuple[float, float]]) -> float:
    """
    Fit a quadratic to E(V) and estimate B = V * d^2E/dV^2 at the minimum-energy volume.
    Returns bulk modulus in GPa. Uses minimal machinery to keep dependencies light.
    """
    if len(ev_rows) < 3:
        raise ValueError("Need at least 3 (V, E) points to fit bulk modulus.")
...
def run_bulk_modulus_oracle(structure_or_path, outdir: str = "qha_out") -> float:
    out = Path(outdir); out.mkdir(parents=True, exist_ok=True)

    base = _to_atoms(structure_or_path)
    ...
    try:
        ...
        print("phonopy-qha not found on PATH; please install phonopy to run QHA.")
    except subprocess.CalledProcessError as e:
        print(f"phonopy-qha failed with exit code {e.returncode}")
    # Return bulk modulus estimate from the E(V) curve.
    return _fit_bulk_modulus(ev_rows)

if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python calc_bulk_modulus.py your_structure.cif [optional_outdir]")
        sys.exit(1)
    cif = sys.argv[1]
    outdir = sys.argv[2] if len(sys.argv) == 3 else "qha_out"
    run_bulk_modulus_oracle(cif, outdir)
