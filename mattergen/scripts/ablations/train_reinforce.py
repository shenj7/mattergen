#!/usr/bin/env python3
"""Train MatterGen with REINFORCE (ablation: no actor-critic, no PPO clipping).

Usage:
    python -m mattergen.scripts.ablations.train_reinforce \
        --checkpoint checkpoints/mattergen_base \
        --output-dir checkpoints/reinforce_bulk_modulus \
        --num-epochs 100
"""
...
from mattergen.rl import MatterGenActor
from mattergen.rl.reinforce_trainer import REINFORCEConfig, REINFORCETrainer
...
# Reuse reward helpers from train_ddpo
sys.path.insert(0, str(Path(__file__).parent.parent))
from train_ddpo import create_reward_fn, create_mattersim_reward_fn, wrap_reward_top_k


def main():
    parser = argparse.ArgumentParser(description="Train MatterGen with REINFORCE (ablation)")
    ...
    parser.add_argument(
        ...
        help="Used only for the neural-net reward function (not for a critic).",
    )
    parser.add_argument("--output-dir", type=str, default="checkpoints/reinforce_bulk_modulus")
    parser.add_argument("--num-epochs", type=int, default=100)
    parser.add_argument("--batch-size", type=int, default=4)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--kl-coeff", type=float, default=0.1)
    parser.add_argument("--entropy-coeff", type=float, default=0.01)
    parser.add_argument("--baseline-ema", type=float, default=0.99)
    parser.add_argument(
        "--reward-type",
        type=str,
    ...
    print("Creating REINFORCETrainer...")
    ...
    print("=" * 60)
    print(f"Starting REINFORCE training for {args.num_epochs} epochs...")
    print(f"  Batch size:     {args.batch_size}")
    print(f"  LR:             {args.lr}")
    print(f"  KL coeff:       {args.kl_coeff}")
    print(f"  Entropy coeff:  {args.entropy_coeff}")
    print(f"  Baseline EMA:   {args.baseline_ema}")
    print("=" * 60)
