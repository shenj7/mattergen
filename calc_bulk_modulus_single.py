...
def calc_bulk_modulus(cif_path: str, outdir: str = "qha_out") -> float:
    """
    Calculate bulk modulus using QHA workflow.
    Returns the bulk modulus (GPa) at 0K from phonopy-qha output.
    """
    import subprocess
    ...
    # Run phonopy-qha automatically
    print("\nRunning phonopy-qha...")
    yaml_files = [f"thermal_properties.yaml-{s}" for s in suffixes]
    cmd = ["phonopy-qha", "e-v.dat", *yaml_files]
    try:
        subprocess.run(cmd, cwd=out, check=True, capture_output=True)
    except FileNotFoundError:
        raise RuntimeError("phonopy-qha not found on PATH; please install phonopy to run QHA.")
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"phonopy-qha failed with exit code {e.returncode}: {e.stderr.decode()}")
    
    # Parse bulk_modulus-temperature.dat and extract value at 0K
    bulk_mod_file = out / "bulk_modulus-temperature.dat"
    if not bulk_mod_file.exists():
        raise RuntimeError(f"Expected output file not found: {bulk_mod_file}")
    
    bulk_modulus_0k = _parse_bulk_modulus_at_0k(bulk_mod_file)
    print(f"\nBulk modulus at 0K: {bulk_modulus_0k:.4f} GPa")
    return bulk_modulus_0k


def _parse_bulk_modulus_at_0k(filepath: Path) -> float:
    """
    Parse bulk_modulus-temperature.dat and return the bulk modulus at 0K.
    File format: two columns - temperature (K), bulk_modulus (GPa)
    """
    with open(filepath, 'r') as f:
        for line in f:
            ...
            if len(parts) >= 2:
                temp = float(parts[0])
                bulk_mod = float(parts[1])
                if temp == 0.0:
                    return bulk_mod
    raise ValueError(f"Could not find 0K entry in {filepath}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python calc_bulk_modulus_single.py your_structure.cif [optional_outdir]")
        sys.exit(1)
    cif = sys.argv[1]
    outdir = sys.argv[2] if len(sys.argv) == 3 else "qha_out"
    bulk_modulus = calc_bulk_modulus(cif, outdir)
    print(f"\n=== RESULT: Bulk Modulus = {bulk_modulus:.4f} GPa ===")
